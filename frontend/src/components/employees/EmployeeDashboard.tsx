...
import AssignedProjects from "@/components/employees/AssignedProject";
import { notifySuccess, notifyError, notifyDelete } from "@/lib/toast";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from "@/components/ui/dialog";

export default function EmployeeDashboard() {
  const { user } = useAuth();
  const { tasks, selectedTask, setSelectedTask, loading, createTask, updateTask, deleteTask, getTasks, getTaskById } = useTask();

  const [openDialog, setOpenDialog] = useState(false);
  const [activeSection, setActiveSection] = useState<"dashboard" | "assignedProjects" | "attendance">("dashboard");
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [taskToDelete, setTaskToDelete] = useState<Task | null>(null);

  // Fetch tasks when user or section changes
  useEffect(() => {
  ...
  const handleSubmitTask = async (formData: Task) => {
    try {
      if (selectedTask?._id) {
        ...
        await updateTask(selectedTask._id, dataToSend); // No need for optional chaining
  ...
  return (
    <div className="flex min-h-screen bg-white">
      ...
      <div className="flex-1 bg-white p-4 md:p-8 overflow-y-auto">
        ...
        {activeSection === "assignedProjects" && <AssignedProjects />}
